...
class GLRubikCube(private val rubikCubeModel: RubikCube) {
    ...
    init {
        paintFace(FRONT) { index -> index % 9 in 6..8 } // Front face
        paintFace(BACK) { index -> index % 9 in 0..2 }  // Back face
        paintFace(LEFT) { index -> index % 3 == 0 }     // Left face
        paintFace(RIGHT) { index -> index % 3 == 2 }    // Right face
        paintFace(TOP) { index -> index / 9 == 2 }      // Top face
        paintFace(BOTTOM) { index -> index / 9 == 0 }   // Bottom face
    }
    ...
    private fun getCubesFromFace(face: Face): List<GLCube> {
        return when(face) {
            ...
            TOP -> glCubes.filterIndexed { index, _ -> index / 9 == 2 }
            BOTTOM -> glCubes.filterIndexed { index, _ -> index / 9 == 0 }
        }
    }
    ...
    private fun paintFace(face: Face, filter: (Int) -> Boolean) {
        ...
        for ((cube, charColor) in faceCubes.zip(faceColors)) {
            when (face) {
                ...
                TOP -> cube.upColor = toColor(charColor)
                BOTTOM -> cube.downColor = toColor(charColor)
            }
        }
    }
}